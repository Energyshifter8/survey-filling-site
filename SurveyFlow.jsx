...
// Дундаж загвар: intro (Эхлэх) -> consent (Цааш) -> questions (Үргэлжлүүлэх) -> done
// Бодит талбарын нэрс (survey.title, question.text г.м) нь backend-ийн жинхэнэ
// response бүтцээс хамаарна — эндэхийг өөрийн API-тай тулгаад тохируулна уу.
...
export default function SurveyFlow({ shortCode }) {
  const [client] = useState(() => new SurveyClient(shortCode));
  ...
  useEffect(() => {
    (async () => {
      try {
        const surveyData = await client.loadSurvey();
  ...
  async function handleConsentNext() {
    ...
    try {
      ...
      setQuestions(data.items ?? data); // response бүтцээс хамаарна
    ...
    <div className="wrap">
      ...
      {step === STEP.INTRO && (
        <div>
          ...
          {survey?.publisher && <p className="publisher">Судалгаа нийтлэгч: <i>{survey.publisher}</i></p>}
      ...
      {step === STEP.QUESTIONS && q && (
        <div>
          ...
          <div className="options">
            {q.options?.map((opt) => (
              <label key={opt.id} className={`option ${answers[q.id] === opt.id ? "selected" : ""}`}>
                <input
                  type="radio"
                  name={q.id}
